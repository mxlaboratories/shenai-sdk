# Shen.AI Capacitor UI Flow

Simple black-on-white Capacitor example with two SDK UI-flow entry points:

- `Dashboard` initializes a dashboard-only SDK flow.
- `Measurement` initializes the measurement/results/health-risks SDK flow,
  disables dashboard access with `setEnableMeasurementsDashboard(false)`, and
  shows an Open PDF action after the SDK flow finishes.
...
```bash
cd examples/capacitor/capacitor-flow
