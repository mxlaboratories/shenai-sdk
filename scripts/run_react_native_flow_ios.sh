#!/usr/bin/env bash
# -------------------------------------------------------------
# Shen.AI – React Native flow example (iOS build helper)
# -------------------------------------------------------------
# Requires: Yarn, Node.js, Xcode 15+, CocoaPods
#
# Usage:
#   SHENAI_API_KEY="<key>" ./scripts/run_react_native_flow_ios.sh                 # lets React Native decide
#   SHENAI_API_KEY="<key>" ./scripts/run_react_native_flow_ios.sh --device "<id>" # explicit physical device
#
# Exit codes:
#   0  success
...
EXAMPLE_DIR="$ROOT_DIR/examples/react-native/react_native_flow"
SDK_DIR="$EXAMPLE_DIR/react-native-shenai-sdk"
NODE_MODULES_DIR="$EXAMPLE_DIR/node_modules"
YARN_CACHE_DIR="${TMPDIR:-/tmp}/shenai-react-native-flow-yarn-cache"
METRO_PORT="${RCT_METRO_PORT:-8082}"
